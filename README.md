...
> **当前是可自行发布的源码，不是已经上线的新网站。** 静态版适合 GitHub Pages，不需要自购云服务器或域名。云账号只有在站点维护者完成 Firebase 配置并验证登录、同步及安全规则后才能启用。页面没有付费入口，也不要求用户绑定支付方式。
...
- 公开页面无需登录，展示并检索打包时的公开书目索引，并链接到现有“每日文献简报”网站。更新公开书目仍需重新构建和发布；它不是实时数据库。
...
- **温度程序**：逐阶段填写时间和温区 A/B，绘制时间—温度示意图。未录入的程序不会被自动猜测。
...
原网站“我的研究”页面的本地源码已增加 **迁移到研究生工作平台** 按钮，会导出 `research-workbench-transfer.json`；该修改尚需推送并发布到原网站，线上才会看到。平台“数据与设置”可直接导入此文件，也接受原有的 `experiment-records.json`、`experiment-records.csv` 和 `paper-projects.json`。旧分组文件只有论文 ID；平台附带原网站公开书目索引用于匹配，匹配不到的条目会标记“待补全文献信息”。
